...
use crate::app::{client, ClientMessage};
...
impl Actor for Chat {
    ...
    fn stopped(&mut self, ctx: &mut Self::Context) {
        ...
        self.db_executor.do_send(database::CloseChat {
            chat_id: self.id,
...
impl Handler<ClientMessage> for Chat {
    ...
    fn handle(&mut self, msg: ClientMessage, ctx: &mut Self::Context) -> Self::Result {
        match msg.event.as_str() {
            ...
            "exit" => match self.members.remove_entry(&msg.fingerprint) {
                Some((fg, addr)) => {
                    ...
                    self.members
                        .clone()
                        .iter()
                        .for_each(move |(fingerprint, addr)| {
                            addr.do_send(ClientMessage {
